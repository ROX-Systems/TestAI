...
### Update 2025-12-17 (Stage 0 repo/CI completion)

- Настроены required checks / branch protection в GitHub (main) под имена job’ов workflow.
- CI остаётся условным: Rust/Node/Flutter проверки выполняются только при появлении соответствующих манифестов.
- Этап 0 (M0) закрыт; Этап 1 (M1) активен.
